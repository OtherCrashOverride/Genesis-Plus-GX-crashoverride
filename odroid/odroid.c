...
EGLDisplay Egl_Intialize(NativeDisplayType display)
{
...
EGLConfig Egl_FindConfig(EGLDisplay eglDisplay, int redBits, int greenBits, int blueBits, int alphaBits, int depthBits, int stencilBits)
{
...
Blit* Blit_Create()
{
    Blit* result = malloc(sizeof(Blit));
    if (!result)
    {
        printf("malloc failed.\n");
        exit(1);
